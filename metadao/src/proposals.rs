...
pub enum ProposalKind {
    ...
    AddMembersToRole { members_id: Vec<AccountId>, role: String },
    ...
    ChangePolicyUpdateRolesPermission { roles: Vec<RolePermissionJson> },
...
impl ProposalKind {
    ...
    pub fn to_policy_label(&self) -> &str {
        match self {
            ...
            ProposalKind::AddMembersToRole { .. } => "add_members_to_role",
            ...
            ProposalKind::ChangePolicyUpdateRolesPermission { .. } => "policy_update_roles_permission",
...
impl Contract {
    ...
    fn internal_execute_proposal(
        ...
        let result = match &proposal.kind {
            ...
            ProposalKind::AddMembersToRole { members_id, role } => {
                let mut new_policy = policy.clone();
                new_policy.add_members_to_role(role, members_id.clone());
                self.policy.set(&VersionedPolicy::Current(new_policy));
                PromiseOrValue::Value(())
            }
            ...
            ProposalKind::ChangePolicyUpdateRolesPermission { roles } => {
                let mut new_policy = policy.clone();
                new_policy.update_roles_permission(roles.clone());
                self.policy.set(&VersionedPolicy::Current(new_policy));
                PromiseOrValue::Value(())
            }
...
impl Contract {
    ...
    pub fn add_proposal(&mut self, proposal: ProposalInput) -> u64 {
        ...
        let deposit_required: u128 = self.fee_metadao.0 + policy.proposal_bond.0;
        ...
        self.total_proposal += 1;
        self.locked_amount += attached_deposit - self.fee_metadao.0;

        // transfer fee metadao
        ...
        &json!({
            ...
            "proposal_type": proposal.kind.to_policy_label().to_string(),
            "kind": json!(proposal.kind).to_string(),
            "description": proposal.description.clone(),
            "proposer": env::predecessor_account_id(),
            "submission_time": env::block_timestamp().to_string(),
            "status": ProposalStatus::InProgress,
