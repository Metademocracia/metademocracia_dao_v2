...
impl Policy {
    ...
    pub fn update_roles_permission(&mut self, roles: Vec<RolePermissionJson>) {
        for i in 0..roles.len() {
            for j in 0..self.roles.len() {
                if &self.roles[j].name == &roles[i].name {
                    let mut new_rol = self.roles[j].clone();
                    new_rol.permissions = roles[i].permissions.clone();
                    new_rol.vote_policy = roles[i].vote_policy.clone();
        
                    let _ = std::mem::replace(&mut self.roles[j], new_rol);
                }
            }
        }
    }
    ...
    pub fn add_members_to_role(&mut self, role: &String, members_id: Vec<AccountId>) {
        for i in 0..self.roles.len() {
            if &self.roles[i].name == role {
                for j in 0..members_id.len() {
                    self.roles[i]
                        .kind
                        .add_member_to_group(&members_id[j])
                        .unwrap_or_else(|()| {
                            env::log_str(&format!("ERR_ROLE_WRONG_KIND:{}", role));
                        }); 
                }
