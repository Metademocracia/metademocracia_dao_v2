...
pub struct MetadaoDAOFactory {
    factory_manager: FactoryManager,
    daos: UnorderedSet<AccountId>,
    wallet_fee: AccountId,
    fee_metadao: U128,
    storage_deposit: U128
}

#[near_bindgen]
impl MetadaoDAOFactory {
    #[init]
    pub fn new() -> Self {
        let this = Self {
            factory_manager: FactoryManager {},
    ...
    pub fn delegate(&mut self, account_id: &AccountId, amount: U128) {
        assert!(env::attached_deposit() >= amount.0, "El deposito es menor al monto indicado");
        ...
        env::log_str(
            &json!({
                ...
                "delegator": env::predecessor_account_id(),
            }).to_string(),
        );
    }

    pub fn get_fee_metadao(&self) -> U128 {
        self.fee_metadao
    }

    pub fn get_wallet_fee(&self) -> AccountId {
        self.wallet_fee.clone()
    }

    pub fn set_fee_metadao(&mut self, fee_metadao: U128) -> U128 {
        assert!(env::current_account_id() == env::predecessor_account_id(), "only metadao");
        self.fee_metadao = fee_metadao;
        self.fee_metadao
    }

    pub fn set_wallet_fee(&mut self, wallet_fee: AccountId) -> AccountId {
        assert!(env::current_account_id() == env::predecessor_account_id(), "only metadao");
    ...
    pub fn create(&mut self, name: AccountId, args: Base64VecU8) {
        ...
        let deposit_required: u128 = self.storage_deposit.0 + self.fee_metadao.0;
        
        assert!(attached_deposit >= deposit_required, "El deposito debe ser mayor a igual a {}", deposit_required);
        ...
        let callback_args = serde_json::to_vec(&json!({
            ...
            "attached_deposit": U128(attached_deposit - self.fee_metadao.0),
    ...
    pub fn on_create(
        ...
        if near_sdk::is_promise_success() {
            ...
            env::log_str(
                &json!({
                    "wallet_dao": account_id,
                    "owner_id": predecessor_account_id.clone()
                }).to_string(),
            );
            Promise::new(self.wallet_fee.clone()).transfer(self.fee_metadao.0);
            true
        } else {
    ...
    /// Only intended for  v2 DAO's created by  factory
...
pub extern "C" fn store() {
    ...
    let contract: MetadaoDAOFactory = env::state_read().expect("Contract is not initialized");
...
mod tests {
    ...
    fn test_create_error() {
        ...
        testing_env!(context
            ...
            .build());
        let mut factory = MetadaoDAOFactory::new();
    ...
    fn test_factory_can_get_current_owner() {
        ...
        testing_env!(context
            ...
            .build());
        let factory = MetadaoDAOFactory::new();
    ...
    fn test_owner_can_be_a_dao_account() {
        ...
        factory.set_owner(AccountId::new_unchecked("bob.dao.near".to_string()));

        assert_eq!(
            factory.get_owner(),
            AccountId::new_unchecked("bob.dao.near".to_string())
        )
    }
